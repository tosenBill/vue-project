...
export const MunuRouter = (routers: AppRouteRecordRaw[], parentPath = "/") => {
  return routers.filter((v) => {
    ...
    if (!meta.hidden) {
      ...
      // 判断是否是完整的url: "https://xxx"
      const fullPath = pathResolve(parentPath, v.path);

      const path =
        onlyOneChild && oneShowingChild && meta.hideChild
    ...
    return meta.alwaysShow && v.children?.length;
  });
};
